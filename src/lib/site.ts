...
export type Product = {
  ...
  rating_count?: number;
  /** Optional bulk/wholesale price breaks — empty/absent means the base price applies at any quantity. */
  price_tiers?: PriceTier[];
};

/**
 * Resolves the correct unit price for a given quantity from a product's
 * base price plus any bulk/wholesale tiers. Tiers are sorted by min_qty and
 * the highest tier whose min_qty is <= qty wins; qty 1 always falls back to
 * the base price if no tier matches. Malformed/empty tiers are ignored so a
 * product with no bulk pricing configured just behaves as before.
 */
export function tierPriceFor(basePrice: number, tiers: PriceTier[] | undefined, qty: number): number {
  const valid = (tiers ?? []).filter((t) => Number(t.min_qty) > 1 && Number(t.price) > 0);
  if (!valid.length) return basePrice;
  const sorted = [...valid].sort((a, b) => Number(a.min_qty) - Number(b.min_qty));
  let price = basePrice;
  for (const t of sorted) {
    if (qty >= Number(t.min_qty)) price = Number(t.price);
  }
  return price;
}

/** The lowest per-unit price a product can be bought at (its best/highest bulk tier), for "as low as" badges. */
export function bestTierPrice(basePrice: number, tiers: PriceTier[] | undefined): number {
  const valid = (tiers ?? []).filter((t) => Number(t.min_qty) > 1 && Number(t.price) > 0);
  if (!valid.length) return basePrice;
  return Math.min(basePrice, ...valid.map((t) => Number(t.price)));
}
