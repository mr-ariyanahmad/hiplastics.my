...
async function reserveStockForOrder(
  ...
  try {
    ...
    for (const r of reserved) {
      await admin.rpc("restore_stock", { p_slug: r.slug, p_qty: r.qty }).catch(() => undefined);
    }
    throw e;
  }
}
...
async function restoreStockForOrder(
  ...
  for (const it of items) {
    await admin.rpc("restore_stock", { p_slug: it.slug, p_qty: it.qty }).catch(() => undefined);
  }
}

async function releaseCouponIfAny(admin: ReturnType<typeof getAdminClient>, couponCode?: string) {
  if (!couponCode) return;
  await admin.rpc("release_coupon", { p_code: couponCode }).catch(() => undefined);
}
...
export async function createOrder(input: CheckoutInput) {
  ...
  await admin.from("cart_snapshots").delete().eq("user_id", userId).catch(() => undefined);
