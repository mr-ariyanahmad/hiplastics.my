...
import { normalizeMyPhone, tierPriceFor, type PriceTier } from "./site";
...
export async function priceCart(lines: CartLine[], state: string, couponCode?: string) {
  ...
  const { data: rows, error } = await admin
    .from("products")
    .select("id, slug, name, price, image_url, image_urls, is_active, is_purchasable, stock, price_tiers")
    .in("slug", slugs);
  ...
  const items = lines.map((l) => {
    ...
    const basePrice = round2(Number(p.price) || 0);
    if (basePrice <= 0) throw new Error(`${p.name} has no price set.`);
    // Bulk/wholesale price break: the highest tier whose min_qty is met by
    // this line's quantity applies to the whole line.
    const unit = round2(tierPriceFor(basePrice, (p.price_tiers as PriceTier[]) ?? [], qty));
...
/**
 * Live stock/availability snapshot for the given cart lines — used by the
 ...
 * right before checkout). Accepts each line's current quantity so it can
 * also return the bulk/wholesale tier price that applies at that quantity —
 * otherwise the cart would keep showing the qty-1 price even after the
 * person's quantity crossed into a cheaper tier.
 */
export async function getCartStockInfo(lines: { slug: string; qty: number }[]) {
  if (!lines.length) return { items: [] as { slug: string; name: string; price: number; basePrice: number; stock: number; available: boolean }[] };
  const admin = getAdminClient();
  const slugs = lines.map((l) => l.slug);
  const { data: rows, error } = await admin
    .from("products")
    .select("slug, name, price, stock, is_active, is_purchasable, price_tiers")
    .in("slug", slugs);
  ...
  if (error) throw new Error(error.message);
  return {
    items: lines.map(({ slug, qty }) => {
      const p = (rows ?? []).find((r) => r.slug === slug);
      if (!p) return { slug, name: slug, price: 0, basePrice: 0, stock: 0, available: false };
      const available = p.is_active !== false && p.is_purchasable !== false;
      const basePrice = round2(Number(p.price) || 0);
      const tiered = round2(tierPriceFor(basePrice, (p.price_tiers as PriceTier[]) ?? [], Math.max(1, qty)));
      return {
        slug,
        name: p.name as string,
        price: tiered,
