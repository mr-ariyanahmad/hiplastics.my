...
type FieldType = "text" | "number" | "bool" | "image" | "images" | "textarea" | "csv" | "select" | "date" | "color" | "tiers";
...
const SCHEMAS: Record<CrudKey, { fields: Field[]; defaults: Record<string, unknown> }> = {
  products: {
    fields: [
      ...
      { key: "price_tiers", label: "Bulk / wholesale pricing (optional)", type: "tiers" },
    ...
    defaults: { name: "", name_ms: "", slug: "", category: "", description: "", description_ms: "", features: [], features_ms: [], image_url: "", image_urls: [], price: 0, price_tiers: [], sku: "", stock: 0, is_purchasable: true, sort_order: 0, is_featured: false, is_active: true },
  },
  categories: {
    fields: [
      { key: "name", label: "Name", type: "text" },
...
const PRODUCT_CSV_COLUMNS = [
  "slug", "name", "name_ms", "category", "description", "description_ms",
  "features", "features_ms", "price", "price_tiers", "sku", "stock",
  "is_purchasable", "image_urls", "sort_order", "is_featured", "is_active",
];

/** Product row (as loaded from the DB) -> one flat CSV row. */
function productToCsvRow(p: Record<string, unknown>): Record<string, unknown> {
  const tiers = Array.isArray(p.price_tiers) ? (p.price_tiers as { min_qty: number; price: number }[]) : [];
  return {
    slug: p.slug ?? "",
    name: p.name ?? "",
    name_ms: p.name_ms ?? "",
    ...
    features_ms: Array.isArray(p.features_ms) ? (p.features_ms as string[]).join("; ") : "",
    price: p.price ?? 0,
    price_tiers: tiers.map((t) => `${t.min_qty}:${t.price}`).join("; "),
    sku: p.sku ?? "",
    stock: p.stock ?? 0,
    is_purchasable: p.is_purchasable ? "true" : "false",
    image_urls: Array.isArray(p.image_urls) ? (p.image_urls as string[]).join("; ") : "",
...
/** One parsed CSV row (all strings) -> a product row ready for bulkUpsertProducts. */
function csvRowToProduct(r: Record<string, string>): Record<string, unknown> {
  const splitList = (s: string) => (s ?? "").split(";").map((x) => x.trim()).filter(Boolean);
  const priceTiers = splitList(r.price_tiers)
    .map((pair) => {
      const [minQtyStr, priceStr] = pair.split(":").map((x) => x.trim());
      return { min_qty: Number(minQtyStr) || 0, price: Number(priceStr) || 0 };
    })
    .filter((t) => t.min_qty > 1 && t.price > 0);
  const truthy = (s: string) => ["true", "1", "yes"].includes((s ?? "").trim().toLowerCase());
  return {
    slug: (r.slug ?? "").trim(),
    name: (r.name ?? "").trim(),
    name_ms: r.name_ms ?? "",
    ...
    description_ms: r.description_ms ?? "",
    features: splitList(r.features),
    features_ms: splitList(r.features_ms),
    price: Number(r.price) || 0,
    price_tiers: priceTiers,
    sku: r.sku ?? "",
    stock: Math.max(0, Math.floor(Number(r.stock) || 0)),
    is_purchasable: r.is_purchasable === undefined || r.is_purchasable === "" ? true : truthy(r.is_purchasable),
    image_urls: splitList(r.image_urls),
    sort_order: Math.floor(Number(r.sort_order) || 0),
...
function Dashboard({ session, onSignOut }: { session: Session; onSignOut: () => void }) {
  ...
  const bulkUpsert = useServerFn(bulkUpsertProducts);
  ...
  function onExportProductsCsv() {
    const rows = ((data?.products ?? []) as unknown as Record<string, unknown>[]).map(productToCsvRow);
    downloadCsv(`hiplastics-products-${new Date().toISOString().slice(0, 10)}.csv`, toCsv(rows, PRODUCT_CSV_COLUMNS));
    toast.success(`Exported ${rows.length} product(s).`);
  }

  async function onImportProductsCsv(file: File) {
    setImportBusy(true);
    try {
      const text = await file.text();
      const parsed = parseCsv(text);
      if (!parsed.length) { toast.error("That CSV has no rows to import."); return; }
      const rows = parsed.map(csvRowToProduct);
      const res = await bulkUpsert({ data: { token: session.token, rows } });
      await refresh();
      const parts = [`${res.created} created`, `${res.updated} updated`];
      if (res.errors.length) parts.push(`${res.errors.length} skipped`);
      toast[res.errors.length ? "error" : "success"](
        `Import done: ${parts.join(", ")}.` + (res.errors.length ? ` First issue — row ${res.errors[0].row}: ${res.errors[0].message}` : ""),
      );
    } catch (e) {
      toast.error(e instanceof Error ? e.message : "Could not import that CSV.");
    } finally {
      setImportBusy(false);
    }
  }
        ...
        <main className="p-4 sm:p-6">
          ...
          {data && (["products", "categories", "industries", "gallery", "downloads", "news", "coupons", "reviews"] as CrudKey[]).includes(tab as CrudKey) && (
            <CrudTable
              ...
              onUpload={onUpload}
              onExportCsv={tab === "products" ? onExportProductsCsv : undefined}
              onImportCsv={tab === "products" ? onImportProductsCsv : undefined}
...
function CrudTable({ tab, rows, categoryOptions, onSave, onDelete, onUpload, onExportCsv, onImportCsv, importBusy }: {
  tab: CrudKey;
    ...
    <div>
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {onExportCsv && (
            <button onClick={onExportCsv} className="inline-flex items-center gap-1.5 rounded-md border border-border px-3 py-1.5 text-xs font-semibold text-foreground hover:bg-secondary">
              <Download className="h-3.5 w-3.5" /> Export CSV
            </button>
          )}
          {onImportCsv && (
            <label className="inline-flex cursor-pointer items-center gap-1.5 rounded-md border border-border px-3 py-1.5 text-xs font-semibold text-foreground hover:bg-secondary">
              {importBusy ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <PackagePlus className="h-3.5 w-3.5" />}
              Import CSV
              <input
                type="file"
                accept=".csv,text/csv"
...
function FieldInput({ field, value, onChange, onUpload }: {
  ...
  if (field.type === "tiers") {
    const rows = Array.isArray(value) ? (value as { min_qty: number; price: number }[]) : [];
    function setRow(idx: number, patch: Partial<{ min_qty: number; price: number }>) {
      const next = rows.map((r, i) => (i === idx ? { ...r, ...patch } : r));
      onChange(next);
    }
    function addRow() {
      onChange([...rows, { min_qty: 10, price: 0 }]);
    }
    ...
    return (
      <div>
        ...
        <p className="mb-2 text-xs text-muted-foreground">
          Charge less per unit when someone buys in quantity — e.g. "10+ units → MYR 6.99 each". Leave empty for normal single pricing.
        </p>
        <div className="space-y-2">
          {rows.map((r, idx) => (
        ...
        <button type="button" onClick={addRow} className="mt-2 inline-flex items-center gap-1.5 rounded-md border border-dashed border-border px-3 py-1.5 text-xs font-semibold text-foreground hover:bg-secondary">
