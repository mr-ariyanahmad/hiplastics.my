import { ArrowRight, ShoppingCart, PackageX, Tags } from "lucide-react";
import { Link } from "@tanstack/react-router";
import { type Product, localizedProduct, bestTierPrice } from "@/lib/site";
...
export function ProductCard({ product }: { product: Product }) {
  ...
  const lowestTierPrice = bestTierPrice(price, product.price_tiers);
  const hasBulkPricing = lowestTierPrice < price;
  return (
    <article className="group flex flex-col overflow-hidden rounded-xl border border-border bg-card shadow-soft transition-all duration-300 hover:-translate-y-1 hover:shadow-elevated">
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        {hasBulkPricing && inStock && (
          <div className="mt-1.5 inline-flex w-fit items-center gap-1 rounded-full bg-primary/10 px-2 py-0.5 text-[11px] font-semibold text-primary">
            <Tags className="h-3 w-3" /> Bulk pricing from {money(lowestTierPrice)}/unit
          </div>
        )}
        {Boolean(product.purchasable) && price > 0 && (
          <button
