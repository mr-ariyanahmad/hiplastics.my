...
-- ============================================================================
-- MIGRATION — 2026-08-16 Bulk / wholesale quantity pricing
-- Optional per-product price breaks, e.g. buy 10+ units and pay less per
-- unit. Stored as a JSON array of {min_qty, price}, e.g.
--   [{"min_qty": 10, "price": 6.99}, {"min_qty": 50, "price": 5.99}]
-- Left as '[]', the product just uses its normal price — nothing breaks if
-- this is never filled in. The base `price` column is always the tier that
-- applies for qty 1.
-- ============================================================================
alter table hiplastics.products add column if not exists price_tiers jsonb not null default '[]';
